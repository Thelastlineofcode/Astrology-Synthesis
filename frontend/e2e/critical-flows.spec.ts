...
test.describe('Critical User Flows', () => {
  ...
  test.describe('Authentication Flow', () => {
    ...
    test('should show error with invalid credentials', async ({ page }) => {
      ...
      const errorVisible = await isVisible(page, 'text=/invalid|error|incorrect/i');
  ...
  test.describe('Fortune Reading Flow', () => {
    ...
    test('should display fortune form and request reading', async ({ page }) => {
      ...
      await expect(page.locator('input[name="birthPlace"], input[placeholder*="location"], input[placeholder*="place"]')).toBeVisible();
      ...
      const hasText = await isVisible(page, 'text=/sun|moon|planet|house|sign/i');
    ...
    test('should validate required birth information', async ({ page }) => {
      ...
      const hasValidationError = await isVisible(page, 
  ...
  test.describe('Advisor Chat Flow', () => {
    ...
    test('should display advisor selection and start chat', async ({ page }) => {
      // Navigate to chat page
      await page.goto('/chat');
      await waitForPageLoad(page);
      
      // Verify advisors are displayed
      const advisorCards = page.locator('[class*="advisor"], [class*="card"], article, section').filter({
      ...
      // Send a test message
      await page.fill('input[placeholder*="message"], textarea[placeholder*="message"]', 
    ...
    test('should allow switching between advisors', async ({ page }) => {
      ...
      const advisors = page.locator('button:has-text("Papa"), button:has-text("Erzulie"), button:has-text("Damballah"), button:has-text("Baron")');
  ...
  test.describe('Profile Management Flow', () => {
    ...
    test('should display reading history', async ({ page }) => {
      ...
      const hasReadings = await isVisible(page, '[class*="reading"], article, li');
      const hasEmptyState = await isVisible(page, 'text=/no reading|empty|get started/i');
